    ...
    function applyViewportMode() {
      ...
      root.classList.toggle("deck-mobile", window.matchMedia("(max-width: 760px)").matches);
      root.classList.toggle("deck-tablet", window.matchMedia("(min-width: 761px) and (max-width: 1180px)").matches);
      root.classList.toggle("deck-short", window.matchMedia("(max-height: 680px)").matches);
    }
    ...
    function animateSlide(slide) {
      ...
      const isCompact = document.documentElement.classList.contains("deck-mobile") ||
      ...
      if (window.gsap) {
        ...
        const nodes = Array.from(slide.querySelectorAll(".flow-node, .signal-card, .commitment"));
        if (nodes.length) {
          gsap.fromTo(nodes, { rotateX: 7 }, {
            rotateX: 0,
            duration: 0.9,
            ease: "back.out(1.6)",
            stagger: 0.05
    ...
    function updateDeckCounter(index) {
      ...
      const label = index === 0
        ? "Caratula"
        : `Diapositiva ${index} de ${contentTotal}`;

      slideCurrent.textContent = index === 0 ? "00" : String(index).padStart(2, "0");
      slideTotal.textContent = index === 0 ? "caratula" : `de ${String(contentTotal).padStart(2, "0")}`;
    ...
    document.querySelector("[data-nav='prev']").addEventListener("click", () => step(-1));
    document.querySelector("[data-nav='next']").addEventListener("click", () => step(1));
    dots.forEach((dot) => {
    ...
    document.querySelectorAll(".flow-node").forEach((node) => {
      node.addEventListener("click", () => {
        document.querySelectorAll(".flow-node").forEach((item) => item.classList.remove("is-focused"));
    ...
    applyTheme(readSavedTheme() || document.documentElement.dataset.theme || "light", false);
    applyViewportMode();
    clearDeckHash();
